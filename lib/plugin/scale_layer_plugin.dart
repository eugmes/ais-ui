...
import 'scale_bar_utils.dart' as util;

class ScaleLayerPluginOptions extends LayerOptions {
  TextStyle textStyle;
  Color lineColor;
  double lineWidth;
...
class ScaleLayerPlugin implements MapPlugin {
  ...
  Widget createLayer(
    ...
    if (options is ScaleLayerPluginOptions) {
  ...
  bool supportsLayer(LayerOptions options) {
    return options is ScaleLayerPluginOptions;
  }
}

class ScaleLayer extends StatelessWidget {
  final ScaleLayerPluginOptions scaleLayerOpts;
  final MapState map;
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<int>(
      ...
      builder: (BuildContext context, AsyncSnapshot<int> snapshot) {
        var zoom = map.zoom;
        var distance =
            scale[max(0, min(scale.length - 1, zoom.round() + 2))].toDouble();
